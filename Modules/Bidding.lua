...
function MonDKP_CHAT_MSG_WHISPER(text, ...)
  ...
  if string.find(text, "!bid") == 1 and core.IsOfficer == true then
    if core.BidInProgress then
      ...
      if cmd == "pass" then -- backwards compatibility
        return
      end
  ...
  if strfind(msg, "EssentialDKP: ") == 1 then
    return true
  elseif strfind(msg, L["DKPAVAILABLE"]) ~= nil and strfind(msg, '%[') ~= nil and strfind(msg, '%]') ~= nil then -- From CommunityDKP
    return true
...
local function StartBidding()
  ...
  if not core.BidInProgress then
    if MonDKP_DB.defaults.EnableAudio and not MonDKP_DB.defaults.DisableCattleAuction then
